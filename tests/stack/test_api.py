...
class TestStackApi(object):
    def test_deploy_job(self, stack_api):
        """
            ...
            stack_api._deploy_job should reset/update an existing job when 1) A databricks_id is given
            2) A databricks_id is not given but one job with the same name exists.

            A StackError should be raised when 1) A databricks_id is not given but there are multiple
        ...
        # TEST CASE 3:
        # stack_api._deploy_job should reset job if a databricks_id not given, but job with same name
        # found
