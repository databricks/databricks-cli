...
INSTALL_OPTIONS = ['jar', 'egg', 'wheel', 'maven-coordinates', 'pypi-package', 'cran-package']
...
def _get_library_from_options(jar, egg, wheel, maven_coordinates, maven_repo, maven_exclusion, # noqa
...
@click.option('--wheel', cls=OneOfOption, one_of=INSTALL_OPTIONS, help=WHEEL_HELP)
...
@eat_exceptions # noqa
@provide_api_client
def install_cli(api_client, cluster_id, jar, egg, wheel, maven_coordinates, maven_repo, # noqa
    ...
    [--jar, --egg, --wheel, --maven-coordinates, --pypi-package, --cran-package].

    Installing a wheel library on clusters running Databricks Runtime 4.2 or higher effectively runs
    ...
    """
    library = _get_library_from_options(jar, egg, wheel, maven_coordinates, maven_repo,
...
@click.option('--wheel', cls=OneOfOption, one_of=UNINSTALL_OPTIONS, help=WHEEL_HELP)
...
def uninstall_cli(api_client, cluster_id, all, jar, egg, maven_coordinates, maven_repo, # noqa
    ...
    if all:
        ...
        return
    library = _get_library_from_options(jar, egg, wheel, maven_coordinates, maven_repo, maven_exclusion,
                                        pypi_package, pypi_repo, cran_package, cran_repo)
