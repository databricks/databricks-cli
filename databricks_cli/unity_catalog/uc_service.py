...
class UnityCatalogService(object):
    ...
    def create_external_location(self, loc_spec, headers=None):
        _data = loc_spec
    ...
    def create_storage_credential(self, cred_spec, headers=None):
        _data = cred_spec
    ...
    def create_table(self, table_spec, headers=None):
        _data = table_spec
        return self.client.perform_query('POST', '/unity-catalog/tables', data=_data,
                                         headers=headers)
    ...
    def update_share_permissions(self, name, perm_spec, headers=None):
        ...
        return self.client.perform_query('POST', '/unity-catalog/shares/%s/permissions' % (name),
                                         data=_data, headers=headers)
    ...
    def create_provider(self, name, comment, recipient_profile, headers=None):
        _data = {
            'name': name,
            'recipient_profile_str': mc_pretty_format(recipient_profile)
        }
        if comment is not None:
            _data['comment'] = comment
    ...
    def update_provider(self, name, new_name, comment, recipient_profile, headers=None):
        ...
        if recipient_profile is not None:
            _data['recipient_profile'] = recipient_profile
        if comment is not None:
            _data['comment'] = comment
