...
class StackApi(object):
    ...
    def _deploy_job(self, resource_properties, databricks_id=None):
        ...
        the databricks_id of the job or updating an existing job. The job is created or updated using
        ...
        :param databricks_id: A dict object containing 'job_id' field of job identifier in Databricks
        server

        :return: tuple of (databricks_id, deploy_output), where databricks_id contains a 'job_id' field
    ...
    def _deploy_workspace(self, resource_properties, databricks_id, overwrite):
        ...
        :return: (dict, dict) of (databricks_id, deploy_output). databricks_id is the physical ID for
        ...
        if databricks_id and databricks_id[WORKSPACE_RESOURCE_PATH] != workspace_path:
            # databricks_id['path'] is the workspace path from the last deployment. Alert when changed
