...
@click.option('--repos-id', cls=OneOfOption, default=None, one_of=ID_OPTIONS, help="Repository ID")
@click.option('--path', cls=OneOfOption, default=None, one_of=ID_OPTIONS, help="Workspace path of the repo object")
...
@eat_exceptions  # noqa
@provide_api_client
def update_repo_cli(api_client, repos_id, branch, tag, path):
    """
    Checks out the repo to the given branch or tag. This call returns an error if the branch 
    or tag doesn't exist.
    """
    content = ReposApi(api_client).update(repos_id, branch, tag)
